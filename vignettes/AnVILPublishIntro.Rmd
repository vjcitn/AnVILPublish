...
Conversion of .Rmd vignettes to .ipynb notebooks uses [Quarto][]
software. It must be available from within _R_, e.g.,

```{r, eval = FALSE}
system2("quarto", "--version")
```

The user must determine if they want their notebooks converted or
rendered into .ipynb notebooks. The difference is that `render`
automatically executes _R_ code blocks and embeds images, while
`convert` will not.

It is also possible to use Python [notedown][2], but support for this
legacy tool will be discontinued; embeded images are not
supported. [notedown][2] must be available from within _R_, e.g.,
